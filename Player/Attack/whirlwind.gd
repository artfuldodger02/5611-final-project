...
func _ready():
	...
	start_tween.tween_property(self, "scale", Vector2(1.92,2.28) * attack_size, 3).set_trans(Tween.TRANS_QUINT).set_ease(Tween.EASE_OUT)
	
	var final_speed = speed
	speed = speed/5.0
	start_tween.tween_property(self, "speed",final_speed,6).set_trans(Tween.TRANS_QUINT).set_ease(Tween.EASE_OUT)
	start_tween.play()
